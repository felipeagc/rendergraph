...
static void
analyzeSpirv(RgExtShaderStage stage, uint32_t *code, size_t code_size, ModuleInfo *module)
{
    ...
    for (Id *id = ids; id != ids + id_bound; ++id)
    {
        ...
        case SpvOpVariable: {
            ...
            case SpvStorageClassInput: {
                ...
                {
                    module->attributes_count = RG_MAX(module->attributes_count, id->location + 1);
    ...
    RgPipelineInfo *info)
{
    ...
    analyzeSpirv(
        RG_EXT_SHADER_STAGE_VERTEX,
        (uint32_t *)vertex_shader->code,
        vertex_shader->code_size / 4,
        &vertex_module);

    ModuleInfo fragment_module;
    analyzeSpirv(
        RG_EXT_SHADER_STAGE_FRAGMENT,
        (uint32_t *)fragment_shader->code,
        fragment_shader->code_size / 4,
        &fragment_module);
    ...
    info->vertex = vertex_shader->code;
    info->vertex_size = vertex_shader->code_size;
    info->vertex_entry = vertex_shader->entry_point;

    info->fragment = fragment_shader->code;
    info->fragment_size = fragment_shader->code_size;
    info->fragment_entry = fragment_shader->entry_point;

    info->vertex_stride = vertex_module.vertex_stride;
